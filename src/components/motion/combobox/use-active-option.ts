...
export function useActiveOption({
  ...
  const moveActive = useCallback((direction: 1 | -1 | "first" | "last") => {
    ...
    setCursor((current) => {
      ...
      // The index is always in range - `last >= 0` above, and the modulo
      // keeps a step inside the list - but this project compiles with
      // `noUncheckedIndexedAccess`, which cannot see that.
      const row = rows[index]
      return row ? { value: row.value, query: options.query } : current
    })
  }, [])
